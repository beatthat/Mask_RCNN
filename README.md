...
This is the matterport Mask_RCNN repo with any updates to make it work on a osx in june 2018.
Changes to date:

1) add cocoapi as a submodule
2) In samples/demo.ipynb
  1) add cocoapi/PythonAPI to the path
  2) fix error for missing keras function by locking keras version to 2.16 (requirements.txt has >= 2.0.8 but versions newer than 2.16 throw errors)
